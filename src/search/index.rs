...
impl SearchIndex {
    ...
    pub fn search(&self, query_str: &str) -> std::result::Result<Vec<SearchResult>, MemoError> {
        ...
        for (score, doc_address) in top_docs {
            ...
            let created_at = retrieved_doc.get_first(self.created_at_field)
            ...
            // titleを取得
            if let Some(title_value) = retrieved_doc.get_first(self.title_field).and_then(|v| v.as_str()) {
                let mut fm = serde_json::Map::new();
                fm.insert("title".to_string(), JsonValue::String(title_value.to_string()));
